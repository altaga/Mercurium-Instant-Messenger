...
export default function TopBar() {
  ...
  const handleClick = useCallback(
    (e) => {
      ...
      else if (!(e.key in EXTERNAL_LINKS)) {
        history.push(e.key);
      }
    },
    [history],
  ...
  return (
    ...
    <ChatModal visible={chatVisible} />
      ...
      <Wrapper>
        ...
        >
          <Menu.Item key={tradePageUrl} style={{ margin: '0 4px 0 20px' }}>
            TRADE
          </Menu.Item>
          {!searchFocussed && (
            <Menu.Item key="/swap" style={{ margin: '0 4px' }}>
              <a
          ...
          {connected && (!searchFocussed || location.pathname === '/balances') && (
            <Menu.Item key="/balances" style={{ margin: '0 4px' }}>
              BALANCES
            </Menu.Item>
          )}
          {connected && (!searchFocussed || location.pathname === '/orders') && (
            <Menu.Item key="/orders" style={{ margin: '0 4px' }}>
              ORDERS
            </Menu.Item>
          )}
          {connected && (!searchFocussed || location.pathname === '/convert') && (
            <Menu.Item key="/convert" style={{ margin: '0 4px' }}>
              CONVERT
            </Menu.Item>
          )}
          {(!searchFocussed || location.pathname === '/list-new-market') && (
            <Menu.Item key="/list-new-market" style={{ margin: '0 4px' }}>
              ADD MARKET
            </Menu.Item>
          )}
          {(connected && !searchFocussed) && (
            <Menu.Item key="/chat" style={{ margin: '0 4px' }} onClick={()=>setChatVisible(!chatVisible)}>
              CHAT
            </Menu.Item>
          )}
          {!searchFocussed && (
